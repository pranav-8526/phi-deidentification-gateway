...
class DeIDGateway:
    ...
    def deidentify(self, raw_text: str, patient_seed: Optional[int] = None,
                    use_shifted_dates: bool = True) -> Tuple[str, str]:
        effective_seed = patient_seed if patient_seed is not None else self.seed
        date_shifter = DateShifter(seed=effective_seed)
        pseudonymizer = Pseudonymizer(date_shifter=date_shifter, use_shifted_dates=use_shifted_dates)
